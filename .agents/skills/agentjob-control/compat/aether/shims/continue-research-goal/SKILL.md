...
1. Preserve the exact user goal text, completion contract, explicit finite
   limits when supplied, omission/null when unlimited, and explicit request
   for fresh recursive discussions.
...
5. Historical v1 and portable v2 records remain reader-only and byte-preserved.
   New approved launches emit canonical v3 state only; never convert a
   historical record in place.
6. Route exact reasoning-profile acceptance and any branch/worktree
   authorization through canonical `continue-goal`. This shim cannot bypass
   combined acceptance, effective-effort verification, or human-only
   non-success terminal validation.
