...
python3 -m unittest discover -s <AGENTJOB_CONTROL_PATH>/tests -v
```

Validation proves only the class it reports. A schema PASS does not establish
runtime correctness; process and control PASS do not establish domain truth.
...
- **Software Engineer:** inspect or change runtime code, schemas, stores,
  adapters, tests, and commands while preserving compatibility.
