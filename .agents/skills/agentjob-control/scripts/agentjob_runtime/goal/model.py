...
GOAL_SCHEMA_VERSION = "sys4ai.continue-goal.v1"
...
PHASES = frozenset(NONTERMINAL_PHASES | TERMINAL_PHASES)
...
NORMAL_TRANSITIONS = frozenset(
    ...
    | {("step_active", terminal) for terminal in TERMINAL_PHASES - {"terminal_complete"}}
    | {("step_verified", terminal) for terminal in TERMINAL_PHASES - {"terminal_complete"}}
)
...
def validate_goal_record(record: Mapping[str, Any]) -> None:
    schema = Path(__file__).resolve().parents[3] / "schemas" / "goal-state.schema.json"
    ...
    if record["state"]["phase"] in ABSORBING_TERMINALS and record["state"]["active_lease"]:
...
def transition_allowed(old_phase: str, new_phase: str, *, recovery: bool = False) -> bool:
    if recovery:
        return (
