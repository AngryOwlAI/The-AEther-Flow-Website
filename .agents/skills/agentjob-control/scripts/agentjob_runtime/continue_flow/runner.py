...
def _preflight_fingerprint(preflight: ContinuePreflight) -> str:
    ...
    return preflight.fingerprint or content_sha256(
        json_value(preflight.as_dict())
    )
...
def run_continue(
    ...
    *,
    ...
    machine_boundary = boundary not in {
    ...
    if machine_boundary:
        ...
        if (
            ...
            or not preflight.boundary.get("task_id")
        ):
            return ContinueInvocation(
                preflight.as_dict(), None, None, None, routed
            )
    try:
        loaded = load_config(
            project_root,
