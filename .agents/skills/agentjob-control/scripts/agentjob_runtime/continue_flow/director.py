...
def resolve_director_packet(
    ...
    *,
    ...
    if boundary != "director_decision_required":
        return DirectorOutcome(
            "protected_stop",
            boundary,
    ...
    if not task_id or expected_revision is None:
        raise RecordValidationError("new Director packet requires task ID and expected revision")
    if not routes:
        return DirectorOutcome(
            "protected_stop",
            boundary,
            "director.no_candidate_route",
            str(task_id),
        )
