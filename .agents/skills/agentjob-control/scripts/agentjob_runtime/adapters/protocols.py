...
class ThreadExecutionProfile:
    ...
    def __post_init__(self) -> None:
        if not self.reasoning_effort.strip():
            raise ValueError("reasoning effort must be nonblank")
        if self.environment_mode != "reuse_bound_checkout":
            raise ValueError("automatic goal relays must reuse the bound checkout")
...
@runtime_checkable
class ThreadExecutionProfileProvider(Protocol):
    """Configure and attest the effective profile of an exact thread."""
